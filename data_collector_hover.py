...
mission pad, hovers while logging coordination and battery data,
and lands.
"""
...
def output_paths(experiment_id, configs):
    ...
    plot_path = plots_dir / f"{safe_name(experiment_id)}_{run_id}_all_battery_lines.png"
    return run_id, experiment_dir, coordination_path, battery_path, drone_paths, plot_path
    ...
    with path.open("r", newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            if not drone_name:
                continue
            try:
                hover_time = float(row.get("hover_elapsed_time", ""))
                battery = float(row.get("battery", ""))
            except (TypeError, ValueError):
                continue
            series.setdefault(drone_name, []).append((hover_time, battery))
...
def generate_battery_line_plot(coordination_path, output_path, experiment_id, run_id):
    ...
    ax.set_title(f"{experiment_id} {run_id}: battery percentage during 150s hover")
    ax.set_xlabel("Hover time (s)")
...
def run_collection(experiment_id, hover_duration=HOVER_DURATION_SEC):
    ...
    run_id, experiment_dir, coordination_path, battery_path, drone_paths, battery_plot_path = output_paths(experiment["experiment_id"], configs)
    output_files = run_output_files(coordination_path, battery_path, drone_paths) + [battery_plot_path]
    run_completed = False
    cleanup_done = False
