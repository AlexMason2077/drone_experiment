...
5. Plot total charging time for 1 / 2 / 3 charging pads.
6. Click a legend item to isolate that dataset; click again to restore all.
"""
...
DEFAULT_SUMMARY = os.path.join(OUTPUT_DIR, "hover_column_summary.csv")
RESET_MARKER_FILE = os.path.join(OUTPUT_DIR, "calculate_time_column_reset_marker.txt")
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "calculate_time_column_history.csv")
OUTPUT_PNG = os.path.join(OUTPUT_DIR, "calculate_time_column_overview.png")

DRONE_ORDER = ["drone_front", "drone_middle", "drone_back"]
DRONE_LABELS = {
    "drone_front": "Front",
    "drone_middle": "Middle",
    "drone_back": "Back",
...
DRONE_NAME_ALIASES = {
    "drone_left": "drone_front",
    "drone_middle": "drone_middle",
    "drone_right": "drone_back",
    "drone_front": "drone_front",
    "drone_back": "drone_back",
}
...
def parse_trial_data(rows, display_trial_id):
    drone_rows = []
    for row in rows:
        drone_name = DRONE_NAME_ALIASES.get(row.get("drone_name", ""), row.get("drone_name", ""))
...
def build_schedule(drone_rows, pad_count):
    ...
    charge_needs = sorted([row["battery_drop"] for row in drone_rows], reverse=True)
