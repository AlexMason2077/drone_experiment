"""Audit the spacing-only model against fixed and unrestricted references."""
...
DEFAULT_ROOT = (
    ...
    / "controlled_k_sweep"
    / "ablation"
    / "spacing_only_current_configuration"
)
...
def _metrics(
    ...
    return {
        ...
        "aggregate_time_improvement_vs_fixed_percent": float(
            100.0 * np.sum(savings) / np.sum(fixed)
        ),
        "mean_per_state_improvement_vs_fixed_percent": float(
            np.mean(100.0 * savings / fixed)
        ),
...
def audit(states_path: Path, costs_path: Path, predictions_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if not np.isfinite(selected).all():
        raise RuntimeError("Model selected an infeasible spacing")
    ...
    report: dict[str, object] = {
        ...
        "exact_spacing_only_reference": _metrics(restricted, fixed, restricted, full),
        "guard_definition": (
            "Compare the model proposal with the current configuration and keep "
            "the lower exact predicted mission cost."
        ),
    }
    rows = states[
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--predictions", type=Path, default=DEFAULT_ROOT / "model" / "independent_predictions.csv"
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_ROOT / "model" / "spacing_only_ablation_audit.json",
    )
    return parser
