"""
Hover front experiment for three Tello drones.

Experiment plan
1. Drone 101 hovers above mission pad 2.
2. Drone 102 hovers above mission pad 1.
3. Drone 103 hovers above mission pad 3.
4. All three drones take off together, move to 80 cm hover height, and hold for 60 seconds.
5. After the hover window ends, all three drones land together.
6. The script logs mission pad position, global position, battery, and summary statistics.
"""
...
SUMMARY_FILE = os.path.join(OUTPUT_DIR, "hover_front_summary.csv")

PAD_SEQUENCE = [1, 2, 3, 4, 5, 6, 7, 8]
DRONE_LAYOUT = [
    {"name": "drone_left", "role": "left", "suffix": "102", "start_pad": 1},
    {"name": "drone_middle", "role": "middle", "suffix": "101", "start_pad": 2},
    {"name": "drone_right", "role": "right", "suffix": "103", "start_pad": 3},
]

EXPERIMENT_TYPE = "hover_front"
...
def main():
    ...
    log_file = os.path.join(OUTPUT_DIR, f"hover_front_{timestamp}.csv")
    with open(log_file, "w", newline="") as f:
        csv.writer(f).writerow(CSV_COLUMNS)

    print("\nHover front experiment")
    print("  drone_left   -> 192.168.0.101 -> pad 1")
    print("  drone_middle -> 192.168.0.102 -> pad 2")
    print("  drone_right  -> 192.168.0.103 -> pad 3")
    ...
    try:
        ...
        for i, tello in enumerate(swarm.tellos):
            ...
            if not wait_for_expected_pad(tello, cfg["start_pad"], timeout=6.0):
                raise RuntimeError(
        ...
        print(f"\nHovering for {HOVER_DURATION_SEC} seconds...")
        active_hover_hold(swarm, HOVER_DURATION_SEC)
