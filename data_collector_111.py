...
BATTERY_WINDOW_LOW_PERCENT = 30
BATTERY_WINDOW_HIGH_PERCENT = 70
...
def fly_node_to_node(swarm, configs):
    ...
    print(
        f"Flying all drones {distance_cm} cm along {direction_text} at "
        f"{NODE_FLIGHT_SPEED_CM_S} cm/s as {segments} node-to-node segments...",
        flush=True,
    )
    for config in configs:
        print(
    ...
    for segment_idx in range(segments):
        set_phase_all(f"node_segment_{segment_idx + 1}")
        print(f"  Formation segment {segment_idx + 1}/{segments}: all drones advance one node.", flush=True)
        for config in configs:
            current_row = config["grid_row"] + config["node_row_direction"] * segment_idx
            current_pad = pad_at_physical_row(config, current_row)
            next_pad = pad_at_physical_row(config, current_row + config["node_row_direction"])
            print(f"    {config['name']}: pad {current_pad} -> pad {next_pad}", flush=True)

        run_swarm_parallel_checked(
            swarm,
            f"Formation segment {segment_idx + 1}",
            lambda i, tello: fly_synchronized_node_segment(
                tello,
                configs[i],
                configs[i]["grid_row"] + configs[i]["node_row_direction"] * segment_idx,
                speed=NODE_FLIGHT_SPEED_CM_S,
            )
        )
        time.sleep(0.8)
        print(f"  Formation segment {segment_idx + 1}/{segments} complete.", flush=True)
...
def run_collection(experiment_id):
    ...
    try:
        ...
        while high_battery:
            ...
            print(
                "Press Enter to discharge high-battery drones to 70% before the experiment, or type skip to take off directly...",
                flush=True,
            )
